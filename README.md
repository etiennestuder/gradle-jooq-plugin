...
```groovy
jooq {
  version = '3.16.3'  // the default (can be omitted)
...
```kotlin
jooq {
  version.set("3.16.3")  // the default (can be omitted)
...
jooq {
    version = '3.16.3'  // default (can be omitted)
...
jooq {
    version.set("3.16.3")  // default (can be omitted)
...
Each configuration `create()`ed under the `jooq { configurations { } }` block presents the script author with an opportunity to customize a tree of JAXB elements drawn from `jooq-meta` which encodes the [JOOQ Codegen XML Schema](https://www.jooq.org/xsd/jooq-codegen-3.13.0.xsd).

For each configuration so configured, this tree is serialized to a temporary XML file and passed as a command-line argument to `org.jooq.codegen.GenerationTool` for code generation.

Before your Kotlin DSL Gradle script configures it, each configuration's JAXB tree is pre-populated as follows:
...
```kotlin
jooqConfiguration.apply {  // this: org.jooq.meta.jaxb.Configuration
    logging = Logging.WARN  // simple assignment of single-valued property of o.j.m.j.Configuration
    jdbc.apply {     // 'jdbc' property was previously instantiated for you 
...
In the example below, `forcedTypes` is an initially-empty `List<org.jooq.meta.jaxb.ForcedType>` member of
...
Familiarity with the [JOOQ Codegen XML Schema](https://www.jooq.org/xsd/jooq-codegen-3.13.0.xsd) and the [`org.jooq.meta.jaxb` package](https://github.com/jOOQ/jOOQ/tree/main/jOOQ-meta/src/main/java/org/jooq/meta/jaxb)
